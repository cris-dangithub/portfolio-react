...
const Background = () => {
  ...
  const generateQuantity = () => {
    ...
    setQuantityKites(
      Math.floor(window.innerWidth / widthRelation) + baseQuantity
    );
  };
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (quantityKites && kites === undefined) {
      ...
      for (let i = 0; i < quantityKites; i++) {
        ...
        objStyle.top = `${
          rootContainer.getBoundingClientRect().bottom * Math.random()
        }px`;
        objStyle.right = `${
          rootContainer.getBoundingClientRect().right * Math.random()
        }px`;
        objStyle.zIndex = `${Math.random() < 0.5 ? 400 : 600}`;
        objStyle.transform = `${
          objStyle.zIndex === "400" ? "scale(0.5)" : ""
        } rotate(-35deg)`;
    ...
    if (kites) {
      ...
      kitesHTMLArray.forEach((kiteHTML) => {
        kiteHTML.addEventListener("transitionend", (e) => {
          const condition2 =
            rootContainer.getBoundingClientRect().bottom -
              kiteHTML.getBoundingClientRect().bottom >
            -100;
